...
class PiperTextToSpeech(AbstractTextToSpeech):
    ...
    def __init__(self, logger: logging.Logger = DEFAULT_LOGGER):
        ...
        if not os.path.exists(archive_path):
            self.logger.info(f"Downloading {archive_name}...")
            wget.download(archive_url, out=archive_path)
            self.logger.info("Download complete.")
        else:
            self.logger.debug(f"{archive_name} already exists; skipping download.")

        # 2) Extract if needed
        piper_dir = os.path.join(base_dir, "piper")
        if not os.path.isdir(piper_dir):
            self.logger.info(f"Extracting {archive_name} into {base_dir}/ …")
            with tarfile.open(archive_path, "r:gz") as tf:
                tf.extractall(path=base_dir)
            self.logger.info("Extraction complete.")
        else:
            self.logger.debug("piper directory already exists; skipping extraction.")
        ...
        if os.path.exists(piper_bin):
            ...
            self.logger.debug(f"Set executable bit on {piper_bin}.")
        else:
            self.logger.warning(f"{piper_bin} not found!")
        ...
        for fname, url in files.items():
            ...
            if not os.path.exists(dest):
                ...
                self.logger.debug(f"{fname} already exists; skipping download.")
