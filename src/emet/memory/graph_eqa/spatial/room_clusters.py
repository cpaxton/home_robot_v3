...
"""Room naming, stamps, and prompt helpers for GraphEQA.

Geometry lives in :mod:`emet.memory.graph_eqa.room_clustering` (``partition`` /
``proximity``). This module is the facade: room *names* come from VLM stamps
(router ``current_room`` and close-look investigate evidence) and explicit room
words in labels — not furniture-class heuristics (chair/table → dining).
"""
...
from emet.memory.graph_eqa.room_clustering.proximity import (
    RoomCluster as RoomCluster,
)
from emet.memory.graph_eqa.room_clustering.proximity import (
    cluster_object_nodes as cluster_object_nodes,
)
...
DEFAULT_ROOM_LINK_RADIUS_M = DEFAULT_LINK_RADIUS_M
...
def question_target_rooms(question: str) -> set[str]:
    ...
    try:
        ...
        pass
    try:
        from emet.memory.graph_eqa.labels import question_stem_for_keywords
