...
from emet.memory.graph_eqa.agentic.action_history import ActionHistoryEntry, resolve_action_progress_mode
...
from emet.memory.graph_eqa.agentic.policy import EvidencePolicy
from emet.memory.graph_eqa.agentic.types import AnswerEvidenceRecord, FinalAnswerDecision, PlaceInspectRecord
from emet.memory.graph_eqa.graph_memory import NavHypothesis, VerifyResult
from emet.memory.graph_eqa.spatial.room_clusters import resolve_room_policy
...
def _handle_inspect_graph(self, args: dict[str, Any]) -> dict[str, Any]:
    return self._tool_inspect_graph()


def _handle_explore_frontier(self, args: dict[str, Any]) -> dict[str, Any]:
    return self._tool_explore_frontier(
        str(args.get("toward") or ""),
        frontier_id=str(args.get("frontier_id") or ""),
    )


def _handle_investigate(self, args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return _tool_obs_id_error(self, raw_obs_id)
    return self._tool_investigate(
        obs_id,
        tool_name=str(args.get("_tool_name") or "investigate"),
...
def _handle_verify_siglip(self, args: dict[str, Any]) -> dict[str, Any]:
    ...
    return self._tool_verify_siglip(str(args.get("phrase") or ""), obs_id)


def _handle_submit_answer(self, args: dict[str, Any]) -> dict[str, Any]:
    return self._tool_submit_answer(str(args.get("answer") or ""))


def _handle_finish(self, args: dict[str, Any]) -> dict[str, Any]:
...
TOOL_HANDLERS: dict[str, Any] = {
    ...
    "navigate_to_obs": _handle_investigate,
    "look_around": _handle_look_around,
    "capture_and_update": _handle_capture_and_update,
...
def handle_tool(self, name: str, args: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if handler is None:
        ...
        if name in ("investigate", "navigate_to_obs"):
            args = {**args, "_tool_name": name}
        out = handler(self, args)
