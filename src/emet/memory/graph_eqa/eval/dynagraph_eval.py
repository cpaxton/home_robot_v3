...
def gt_metrics(episode_dir: Path) -> dict[str, Any]:
    ...
    from emet.memory.graph_eqa.eval.sim_ground_truth_graph import (
...
def compute_dynagraph_eval(
    ...
    *,
    ...
    if eqa_rows:
        from emet.memory.graph_eqa.eval.question_bank import score_eqa_results

        eqa_section = score_eqa_results(eqa_rows, episode_dir=episode_dir)
    elif questions_path:
        from emet.memory.graph_eqa.eval.question_bank import load_question_bank
