...
"""InternVL (HF-native ``InternVLForConditionalGeneration``) multimodal client."""
...
DEFAULT_INTERNVL_HF_MODEL_ID = "OpenGVLab/InternVL3-14B-hf"


def _content_to_internvl_messages(
    user_content: str | list[Any],
    *,
    system_prompt: str | None,
    image: Any | None,
) -> list[dict[str, Any]]:
    """Build chat messages for InternVL image-text-to-text (HF chat template)."""
...
class InternVLClient(AbstractVLLMClient):
    """Multimodal InternVL via HF-native ``*-hf`` checkpoints (e.g. InternVL3-14B-hf)."""

    def __init__(
        self,
        prompt: str | AbstractPromptBuilder | None = None,
        prompt_kwargs: dict[str, Any] | None = None,
        hf_model_id: str = DEFAULT_INTERNVL_HF_MODEL_ID,
        max_tokens: int = 4096,
        device: str = "cuda",
        ...
        # Prefix-cache / image knobs accepted for factory parity; unused for now.
        _ = (cache_system_prefix, max_cached_prefixes, image_max_side, image_max_pixels)
        ...
        print(
            f"Loading InternVL: {hf_model_id} (quant={quantization or 'none'}, device={device})",
            flush=True,
        )
        ...
        try:
            ...
            if not env_allow_cpu_vlm():
                raise RuntimeError(
                    f"InternVL GPU load failed ({e}). Refusing silent CPU fallback. "
                    "Free VRAM or set EMET_ALLOW_CPU_VLM=1."
                ) from e
            logger.warning(
                "InternVL GPU load failed (%s); EMET_ALLOW_CPU_VLM=1 — retrying bf16 on CPU (slow).",
                e,
            )
            self._device = "cpu"
        ...
        print(f"  InternVL devices: {summarize_model_devices(self.model)}", flush=True)
        assert_cuda_placement(
            self.model,
            requested_device=self._device,
            model_label=f"InternVL ({hf_model_id})",
        )

    @property
    def canonical_model_key(self) -> str:
        q = self._quantization or "none"
        return f"internvl:{self._resolved_hf_model_id}:{self._device}:{q}"
    ...
    def generate_multimodal(
        ...
        *,
        ...
        if verbose:
            print(f"InternVL response (truncated): {text_out[:500]}...")
