...
def test_run_agent_with_robot_llm_repeated_no_tools_no_executor_calls():
    """LLM turns with tool_calls=[] should not execute robot actions."""
    last_robot: dict[str, MagicMock] = {}
    last_exec: dict[str, MagicMock] = {}

    class _Client:
        def __call__(self, text: str, verbose: bool = False, **kwargs):
            return '{"tool_calls": [], "message": "ok"}'
    ...
    def fake_executor(robot, parameters, **kwargs):
        ex = _make_executor(robot, parameters, **kwargs)
        last_exec["e"] = ex
        return ex

    with (
        patch("emet.agent.loop.StretchZmqClient", side_effect=fake_stretch),
        patch("emet.agent.loop.DynamemTaskExecutor", side_effect=fake_executor),
        patch("emet.agent.loop.get_memory_backend", return_value=_DummyMemBackend()),
        patch("emet.agent.loop.get_llm_client", return_value=_Client()),
        patch("emet.agent.loop.print_embodied_model_report"),
        patch("emet.agent.loop.print_memory_view_help_on_quit"),
    ):
        run_agent_with_robot(
            robot_ip="127.0.0.1",
    ...
    assert last_robot["r"].stop.call_count == 1
    assert last_exec["e"].__call__.call_count == 0
