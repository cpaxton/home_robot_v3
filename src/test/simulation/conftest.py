...
_ROBOCASA_ASSETS_FIXTURES = (
    _PROJECT_ROOT / "third_party" / "robocasa" / "robocasa" / "models" / "assets" / "fixtures"
)
...
def _ensure_robocasa_assets() -> None:
    ...
    if not (robocasa_root / "robocasa").exists():
        pytest.skip(
            "third_party/robocasa not found. Run: emet install sim"
        )
    if _ASSETS_OK:
        return
    ...
    if not script.exists():
        pytest.fail(
            "Robocasa assets missing and scripts/download_robocasa_assets.py not found. "
            "Run: emet install sim"
        )
