...
def _stretch_molmo_dynamem_skip_reason() -> str | None:
    ...
    if not _truthy("RUN_STRETCH_MOLMO_DYNAMEM"):
        return "Heavy stack; set RUN_STRETCH_MOLMO_DYNAMEM=1 to run."
    return None


_STRETCH_MOLMO_DYNAMEM_SKIP = _stretch_molmo_dynamem_skip_reason()


@pytest.mark.timeout(600)
@pytest.mark.skipif(
    _STRETCH_MOLMO_DYNAMEM_SKIP is not None,
    reason=_STRETCH_MOLMO_DYNAMEM_SKIP or "skipped",
)
def test_stretch_molmospaces_dynamem_floor_map_covers_collision_clip_and_populates_grid():
    ...
    try:
        ...
        if not _wait_for_port("127.0.0.1", recv_port, timeout_sec=90):
            ...
            pytest.fail(f"Stretch MuJoCo server did not open recv socket on {recv_port}. stderr:\n{err}")
