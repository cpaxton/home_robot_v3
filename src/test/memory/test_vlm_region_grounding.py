...
def test_grounding_prompt_locates_query_not_task_context():
    from emet.memory.vlm_region_grounding import select_vlm_region
    ...
    prompt = audit["prompt"]
    assert f"Locate the visible object {question!r}" not in prompt
    assert "Locate the visible object 'towels'" in prompt
    assert "Verification context" in prompt
    assert "hypothetical" in prompt


def test_box_only_grounding_prompt_locates_query_not_task_context():
    from emet.memory.vlm_region_grounding import select_vlm_region
...
def test_grounding_prompt_omits_redundant_context_when_description_matches_query():
    from emet.memory.vlm_region_grounding import select_vlm_region

    client = Mock(return_value='{"verified":true,"box":[200,200,800,800],"point":[500,500]}')
    _, audit = select_vlm_region(np.zeros((8, 8, 3), dtype=np.uint8), "mug", "mug", client=client)
    assert "Verification context" not in audit["prompt"]
    assert "Locate the visible object 'mug'" in audit["prompt"]


def test_grounding_target_splits_query_from_context():
    from emet.memory.vlm_region_grounding import _grounding_target

    assert _grounding_target("towels", "Which of the following is in the bathroom? A) towels B) toilet") == (
        "towels",
        "Which of the following is in the bathroom? A) towels B) toilet",
    )
