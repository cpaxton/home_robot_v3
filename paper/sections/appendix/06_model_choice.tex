...
fp32). The non-VLM stack (Habitat, voxel map, SigLIP) holds $\sim$7\,GB, leaving roughly
13--15\,GB of headroom; 4-bit NF4 quantization~\citep{dettmers2023qlora} brings 8--9B-class
VLMs comfortably inside it, while 27--32B-class models ($\sim$15--18\,GB at int4) do not fit
...
\begin{enumerate}
  ...
  \item \textbf{$\sim$8B int4 is the practical ceiling} on a 24\,GB GPU shared with
  Habitat and SigLIP; upsizing from 3B bf16 to 8B int4 yielded the largest accuracy gain
  we measured (+3/6 on canonical-6).
