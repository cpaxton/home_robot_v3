...
| Piece | Role |
|-------|------|
| `emet.controller.dynamem` | `DynamemController` facade. Perception, describe, look, nav, EQA live in sibling modules and are bound onto the class. `get_voxel_map` lives in `mapping.py`. Import path `controller_dynamem` is a shim. |
...
| `mapping/voxel/dynamem_eqa.py` | Mixin: classic voxel `query_answer` / frontiers / `get_active_image_descriptions`. |
| `emet.app.graph_nav_cli` | Shared Click CLI for **Dynagraph** and **LazyGraph**. `configure_graph_nav` sets the controller for process lifetime; `with configure_graph_nav(...)` restores the previous selection. |
