...
| MJCF mesh head frozen / map vs model misaligned | ZMQ ``joint_head`` for display is inferred from ``camera_pose`` FK on hardware. Sim ``head_visual`` sits at the neck pivot; hardware replay shifts it forward (~70 mm) so the head STL wraps the stereo cameras. Sim meshes face **−base Y**; hardware replay applies **+90° Z** on the body. Meshes are dark semi-transparent (not flat white); the red base sphere/arrow are off by default. Run ``uv run python scripts/debug_innate_mars_head_align.py --ip <host>``. |
