...
**Expected accuracy deltas:** Location letter overrides (`equip_letter` → `img_letter` → abstain+`memory_letter`) and CONFIRMED_MEMORY injection are **intentional eval levers**. Scores vs GraphEQA-only or override-off ablations can move ± several points on annotated-37 / paper-113; treat those deltas as expected when reporting HM-EQA, and always pair the number with the fingerprint. Do **not** compare override-on Dynagraph to an older no-merge GE-only JSONL without noting the harness change.

**Accept-vs-fix hard cases:** Q16 (pillow/sofa labeling) and exploration-limited Q18/Q31/Q57 can still flake; Dynagraph’s static HM-EQA claim is *no GE-only memory collapse*, not 100% on every search MCQ. True world-change wins stay on the Robocasa/Molmo dynamic-exploration track.
