...
- **Attached RGB:** `query_answer` shows **full camera frames** for FIND / Action-pinned observations (the graph is an index into stored views, not the count). **Visual FIND** is DynaMem retrieve: question Qwen names phrases → voxel SigLIP `find_all_images` (top‑k, not argmax) mapped onto graph RGB, spread in XY. Habitat snapshots those ranks in `prepare_dynagraph_vram_for_eqa` *before* GPU SigLIP is dropped for Qwen — `query_answer` cannot `encode_text` after that release. An optional live-frame Qwen highlight (“what in this image could answer?”) adds extra SigLIP queries when phrases retrieve poorly (e.g. “what time is it” vs a wall clock). YoloE class strings are leftover recall, not Image 1. GRAPH_COUNT lists retrieved views; list length is never the answer. Detector close-ups remain labeled extras of an already-attached scene. SigLIP only proposes RGB — it is not an “object confirmed” oracle. If Image 1 is already that FIND view and the VLM is unconfident with no action, the controller **stays** when the robot is already close, or **approaches the SigLIP xyz** when it is still far, instead of frontier-chasing or `Action:` a YoloE node id.
