...
| `eqa.attempt_ledger` | **off** | When on (`true` or `{enabled: true}`), `GraphEQAMemory.record_nav_attempt` also appends structured `AttemptRecord` rows (action outcomes) alongside the existing per-node `nav_attempts` / `nav_failures` counters. Default off so HM-EQA / OVMM paper paths are unchanged. Env: `EMET_EQA_ATTEMPT_LEDGER`. See [plans/2026-08-08_embodied_agent_planning.md](plans/2026-08-08_embodied_agent_planning.md). |
