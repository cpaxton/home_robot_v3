...
Controller code lives in [`emet.controller.dynamem`](../src/emet/controller/dynamem/) (`DynamemController` facade; `controller_dynamem` is a shim). Voxel localize / classic EQA are mixins on [`voxel_dynamem.py`](../src/emet/mapping/voxel/voxel_dynamem.py). Dynagraph and LazyGraph share [`graph_nav_cli.py`](../src/emet/app/graph_nav_cli.py). See [graph_memory.md](graph_memory.md#voxel-nav-stack-dynamem--dynagraph--lazygraph).
...
**Important (paper / methods):** low `detection.confidence_threshold` is intentional for **candidate proposal** — instance segments and graph-node seeds prioritize recall over precision. Those proposals are associated, fused, and (when enabled) captioned/queried by the VLM and memory backends. They must **not** be treated as calibrated open-vocab class predictions for user-facing chat. Raising the **mapping** threshold is allowed when metrics support it (spatial recall, OVMM find, graph coverage); do that deliberately. Chat-only filtering uses a separate `describe_confidence_threshold` and does not change proposal recall. The embodied agent tool `describe_scene` prefers a VLM caption or graph/map memory; see [AGENT_RUN.md](AGENT_RUN.md), [graph_eqa.md](graph_eqa.md), and [graph_memory.md](graph_memory.md).
