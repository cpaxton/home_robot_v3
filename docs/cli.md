...
| `emet jobs run --name NAME [-d TEXT] [--need-mib N] [--cpu-safe/--no-cpu-safe] [--gpu-exclusive/--no-gpu-exclusive] [--wait-pid P] -- CMD…` | Start a detached supervisor that self-registers, sets `EMET_JOB_ID`, and runs the command. With `--need-mib`, **cpu-safe** and **gpu-exclusive** default **on**. **gpu-exclusive** additionally wraps the command in a shared **singleflight `flock`** on `~/runs/emet/gpu.lock` (override `EMET_GPU_LOCK`; bound the wait with `EMET_GPU_LOCK_TIMEOUT` seconds, default infinite) — one GPU experiment per box across *all* emet checkouts (v2/v3/v4), so concurrent launches serialize instead of OOM-freezing the machine. The flock auto-releases on job exit or host reboot. |
