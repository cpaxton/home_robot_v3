...
**Presets:** `configs/ovmm/sweeps/` (e.g. `molmo-robocasa`). Explicitly **no** `default_table`. Dynagraph find uses the **same AgenticEQA loop** as HM-EQA (OVMM phrased as questions) plus DynaMem `localize_text` as an investigate card and the scored XYZ (not camera pose). Preset `agentic_find: true`. Ablation of the loop (mapping quality only): `emet ovmm find --oneshot-localize` / `emet ovmm full --oneshot-localize`. Agentic budget: `--agentic-max-rounds` / `--agentic-max-nav-steps` on `find`/`full` (or preset `defaults.agentic_max_rounds` / `agentic_max_nav_steps`). `--via-jobs` sets `EMET_ALLOW_SDPA_ATTN=1` so in-process VL uses SDPA (FA2 has hung with MuJoCo co-resident).
