...
| `EMET_GRAPH_FUSION_CONFIG` | `GraphObjectFusion` | Path to a `graph_object_fusion` YAML overriding the packaged default (`src/emet/config/agents/default_graph_object_fusion.yaml`) for a run. Used by the merge-policy IoU sweep (`scripts/sweep_fusion_iou.sh`) and the strategy comparison (`scripts/eval_merge_strategies.sh`). See [graph_object_fusion.md](graph_object_fusion.md). |
