...
| `EMET_EQA_LOCATION_OVERRIDE_IMAGE_GATE` | `GraphEQAMemory` location MCQ | Default **off** (`eqa.location_override_image_gate`). When on, also gate image-landmark overrides on VLM confidence (stricter; off preserves image-grounded corrections of memory-steered letters). |
