...
| `EMET_EQA_ATTEMPT_LEDGER_MODE` | Grounded Graph Agent / HM-EQA | `off` (default), `shadow`, or `agent`. H2H maps non-off modes to the existing write gate; the mode records whether rows are intended to remain shadow-only or become agent-visible. Also `eqa.attempt_ledger_mode`. |
