...
| `EMET_EQA_ATTEMPT_LEDGER` | `GraphEQAMemory.record_nav_attempt` | Default **off**. When `1`, append structured `AttemptRecord` rows (navigate / verify / pick / place / closer_look outcomes) in graph memory in addition to per-node `nav_attempts` / `nav_failures`. Also `eqa.attempt_ledger` (`true` or `{enabled: true, max_records, persist_absent_claims}`). See [2026-08-08_embodied_agent_planning.md](plans/2026-08-08_embodied_agent_planning.md). |
| `EMET_ATTEMPT_LEDGER_PERSIST_ABSENT` | `GraphEQAMemory.clear_retracted_nav_claims` | When `1`, keep close-look ABSENT claim blacklists across questions (ledger rows always persist). Default **off**. Also `eqa.attempt_ledger.persist_absent_claims`. |
