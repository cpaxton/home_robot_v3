...
| `EMET_EQA_CLOSE_LOOK_CONFIDENCE_GATE` | `GraphEQAMemory.query_answer` | Default **off**. When `1`, downgrade `confidence: true` to false if `CLOSE_LOOK_STATUS` is `resolved=no` on pinned/attached/count views (needs a voxel `close_map`). Also `eqa.close_look_confidence_gate`. |
| `EMET_EQA_CENTER_ZOOM` | GraphEQA `query_answer`, CHAT `describe_scene` / `send_image` | Default **off**. Blind center-crop zoom for clocks/labels when no detector bbox. Count/clock slice (q33/q43/q84) did not improve. `1` re-enables the helper in `eqa_views.center_zoom_crop`. YAML: `eqa.center_zoom`. |
