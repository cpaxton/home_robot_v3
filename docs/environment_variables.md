...
| `EMET_EQA_LOCATION_MISSING_FIND` | `GraphEQAMemory.query_answer` | Default **on** — location MCQs downgrade while an unattached FIND view (`_eqa_find_obs_ids` `count_mcq_only=False`) remains (mirror count, q47 `wall clock`). `0` disables (6/15 vs 4/5 canary). Also `eqa.location_missing_find`. |
| `EMET_EQA_CLOSE_MAP_GATE` | `GraphEQAMemory.query_answer` | Default **on** — location MCQs also require voxel `close_map resolved` (`close_map_catalog_fields` `R_M 0.55` `aimed`) before confident (AB `7/15` vs A `6/15`). `0` disables. Also `eqa.close_map_gate`. |
| `EMET_EQA_IMG_STRICT` | `GraphEQAMemory.query_answer` | Default **off** — require attached image landmark (`_location_letter_from_attached_images`) to match parsed letter before location confident (`5/15`). `1` enables. Also `eqa.img_strict`. |
