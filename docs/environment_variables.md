...
| `EMET_EQA_AGENTIC_CLOSE_LOOK_MULTIVIEW` | `AgenticEQAExecutor` | Default **off** (experimental). DeWorldSG-style temporal evidence: aggregate close-look crops across views of the same target and feed them all to the VLM as extra images before answering, instead of a single (wide + one crop) glance. Helps count / clock questions where one zoomed view can be ambiguous. `1` enables; also `eqa.agentic_close_look_multiview`. Requires `agentic_close_look` on (the crop is only computed for close-look questions). |
| `EMET_EQA_AGENTIC_SIGLIP_EVIDENCE` | `AgenticEQAExecutor` | Default **on**. Feed the `'<phrase>' similarity=… (present-like/weak)` SigLIP image-text hint into the VLM assess prompt (evidence, not a verdict — the VLM still weighs what it sees). `0` drops the line for an A/B against the pre-close-look baseline. Also `eqa.agentic_siglip_evidence`. |
