...
| `configs/agent_innate_mars.yaml` | innate_mars | Discord + EQA captions; DA3 depth overlay; `agent.name: Herman`; **text router** `caliban:8000` + **remote VL** `mapping.eqa.vl_endpoint` → `caliban:8001`; store on profile with `emet connect save … --config configs/agent_innate_mars.yaml` then `emet run agent --connection herman` |
...
export DISCORD_TOKEN=...
# Preset: text :8000 + VL :8001 on caliban (see docs/llm_serve.md); optional --rerun
