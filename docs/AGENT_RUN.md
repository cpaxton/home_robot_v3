...
| **EQA_EPISODE** | Dynagraph / Habitat `run_eqa` when `eqa.agentic_verify` | `navigate_to_obs`, `explore_frontier`, `look_around`, `verify_siglip`, `submit_answer` / `finish` (`EQA_SKILL_SPECS`) | VLM-assess answerable → submit (or explore `finish`); detectors are proposals only |
