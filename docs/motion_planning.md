...
Before exec, `_filter_unsafe_nav_traj` also checks mid-segments between waypoints. On `aborted_waypoint_timeout`, Dynamem marks the goal in `_habitat_blocked_goals` so multi-goal explore skips it and replans elsewhere.
