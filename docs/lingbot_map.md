...
- **Sim nav:** Robocasa / innate_mars robosuite sim uses holonomic **velocity drive** by default (`teleport_base: false`). Set `EMET_SIM_NAV_TELEPORT=1` for instant base snap (CI/e2e only). With teleport, Stretch client waits use the **world** pose for `nav_world` goals (episode gps alone caused false 10s timeouts).
