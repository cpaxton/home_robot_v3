...
1. **Full-113 dynagraph @ 8B + nav stack** (paper §04 item #1;
   `scripts/run_hmeqa_paper113_h2h.sh`) — the missing paper baseline.
2. **32B int4** holdout-4 agentic → holdout-8 if healthy
   (`--eqa-hf-model-id Qwen/Qwen3-VL-32B-Instruct`); abort on OOM/EGL streak.
3. **Matched static_graph** on the same 113 (isolate Dynagraph memory gains).
