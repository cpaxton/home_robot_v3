...
**Status:** replay-first — loader exists; scored harness TBD.
...
Nothing is **checked into git** for DynaMem replay (no LFS fixtures). Replay works from disk paths you already have under `logs/`.
...
4. **Record3D / DynaBench** — only if we add new files; not required for replay-based eval.
