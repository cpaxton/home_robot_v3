...
"""Download ScanNet meshes for SQA3D embodied replay."""
...
Run embodied episode (after SQA3D + ScanNet data):
  uv run emet sqa3d run-episode --question-id 220602000000 --mock-llm
"""
    )
...
def _run_download_script(
    ...
    *,
    ...
    cmd = [sys.executable, str(script), "-o", str(scannet_root), "--id", scene_id, "--type", file_type]
    proc = subprocess.run(
        cmd,
...
def main() -> None:
    ...
    if args.verify:
        present = sum(1 for s in scenes if scene_assets_present(s, scannet_root))
        print(f"SCANNET_ROOT={scannet_root}")
        print(f"scenes_requested={len(scenes)} present={present}")
        for s in scenes[:20]:
            mesh = scene_mesh_path(s, scannet_root)
            print(f"  {s}: {mesh.is_file()} -> {mesh}")
        if len(scenes) > 20:
    ...
    for scene_id in scenes:
        if scene_assets_present(scene_id, scannet_root):
            print(f"  {scene_id}: already present")
            ok_count += 1
            continue
        if _download_scene(script, scannet_root, scene_id, accept_tos=True):
            ok_count += 1
    print(f"Done: {ok_count}/{len(scenes)} scenes with mesh on disk")
