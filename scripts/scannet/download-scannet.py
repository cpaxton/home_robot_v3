...
def download_tfrecords(in_dir, out_dir):
    ...
    for folder_name in ["hires_tfrecords", "lores_tfrecords"]:
        folder_dir = f"{in_dir}/{folder_name}"
        save_dir = f"{out_dir}/{folder_name}"
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        for split, num_shards in split_to_num_shards.items():
            for i in range(num_shards):
                file_name = f"{split}-{i:05d}-of-{num_shards:05d}.tfrecords"
                url = f"{folder_dir}/{file_name}"
                localpath = f"{out_dir}/{folder_name}/{file_name}"
                download_file(url, localpath)
...
def main():
    ...
    parser.add_argument(
        "--tf_semantic", action="store_true", help="download google tensorflow records for 3D segmentation / detection"
    )
    ...
    parser.add_argument(
        "--skip_existing", action="store_true", help="skip download of existing files when downloading full release"
    )
    ...
    if args.task_data:  # download task data
        ...
        download_file(
            os.path.join(BASE_URL, RELEASE_TASKS, TEST_FRAMES_FILE[0]), os.path.join(out_dir_tasks, TEST_FRAMES_FILE[0])
        )
        ...
        if scan_id not in release_scans and (not is_test_scan or args.v1):
            ...
            out_dir = (
                os.path.join(out_dir_scans, scan_id) if not is_test_scan else os.path.join(out_dir_test_scans, scan_id)
            )
