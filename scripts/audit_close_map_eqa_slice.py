...
def audit_slice(
    ...
    *,
    ...
    for row in rows:
        ...
        if (eqa_iterations is None or planning_steps is None) and bundle.is_dir():
            metrics_path = bundle / "metrics.json"
            if metrics_path.is_file():
                metrics = json.loads(metrics_path.read_text(encoding="utf-8"))
                if eqa_iterations is None:
                    eqa_iterations = metrics.get("eqa_iterations")
                if planning_steps is None:
                    planning_steps = metrics.get("planning_steps")
        entries.append(
            {
                "question_id": qid,
                "correct": correct,
                ...
                "eqa_iterations": int(eqa_iterations) if eqa_iterations is not None else None,
                "planning_steps": int(planning_steps) if planning_steps is not None else None,
                "keywords": keywords,
...
def _print_report(report: dict[str, Any]) -> None:
    ...
    print()
    print(
        f"{'qid':>4} {'ok':>3} {'mconf':>5} {'eqa':>4} {'plan':>5} "
        f"{'proxy_res':>9} {'min_m':>6}  question"
    )
    for e in sorted(report["entries"], key=lambda x: x["question_id"]):
        ...
        eqa_s = str(e.get("eqa_iterations") if e.get("eqa_iterations") is not None else "?")
        plan_s = str(e.get("planning_steps") if e.get("planning_steps") is not None else "?")
        q = ""
        for row in _load_jsonl(Path(report["jsonl"])):
            if int(row.get("question_id", -1)) == e["question_id"]:
                q = str(row.get("question") or "")[:48]
                break
        print(
            f"{e['question_id']:4d} {'Y' if e['correct'] else 'N':>3} "
            f"{'Y' if e['model_confident'] else 'N':>5} {eqa_s:>4} {plan_s:>5} "
            f"{res:>9} {min_s:>6}  {q}"
        )
